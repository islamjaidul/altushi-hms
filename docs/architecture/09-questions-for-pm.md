...
## Clearance sign-off: who signs (2026-08-06 · spec `docs/specs/0056-hr-employment-lifecycle/`)

- **Status:** Open · **Date:** 2026-08-06 · **Spec:** 0056

Module PRD §7.1 requires a clearance checklist "routed to the departments that must sign off (HR,
Accounts, IT, Store, department head), each with a status, a note and an attributable sign-off".
**§11 names no permission for it.**

| # | Question | Why it surfaced | Shipped in 0056 |
|---|---|---|---|
| P30 | **Does each clearing department hold its own sign-off claim?** §11 lists `hr.settlement.manage` for "separation, clearance and final settlement" — one permission for the whole workflow, held by the HR Manager. But "routed to IT" implies IT acts, and an IT lead holds no HR permission at all. | Inventing `hr.clearance.signoff` would be inventing scope (rule 2). Giving IT `hr.settlement.manage` would give them the settlement's money as well, which is the opposite of what routing is for. | An item is signed off by a holder of `hr.settlement.manage`, **or** by the head of the org unit the item is routed to (§11 scoping rule 1's existing shape). In practice HR records each department's response. If the customer wants IT and Store to sign for themselves, that is one new permission and a nav entry — small, but it is scope, so it is the PM's call. |
| P31 | **Is the department list configurable?** 0056 seeds §7.1's five departments onto every separation. | An employer with no store, or with a separate Library or Transport desk, gets a checklist that does not match how they actually clear someone. | The five are fixed, taken from §7.1's own enumeration rather than invented. A configurable checklist is a masters table and a setup screen — worth doing if a second customer asks, not before. |
